...
import { Save, ArrowLeft, Ticket, DollarSign, Calendar, Settings, Loader2 } from 'lucide-react';
...
export default function Edit({ ticket }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        ...
        sale_start_date: ticket.sale_start_date ? ticket.sale_start_date.slice(0, 16) : '',
        sale_end_date: ticket.sale_end_date ? ticket.sale_end_date.slice(0, 16) : '',
        is_active: ticket.is_active as boolean,
        image: null as File | null
    });

    const [imagePreview, setImagePreview] = useState<string | null>(
        ticket.image ? `/storage/${ticket.image}` : null
    );
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Edit Tiket - ${ticket.name}`} />
            <div className="p-4">
                <div className="mb-4 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        <Ticket className="h-5 w-5 text-blue-600" />
                        <h1 className="text-xl font-semibold">Edit Tiket: {ticket.name}</h1>
                    </div>
                    <Button 
                        variant="outline" 
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <p className="text-sm text-muted-foreground">
                                        Stok saat ini: {ticket.stock} tiket
                                    </p>
                                </div>
                            </div>
                        </CardContent>
